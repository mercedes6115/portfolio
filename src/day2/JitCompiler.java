...
public class JitCompiler {
    ...
    // 자바 클래스가 로드되는 과정
    // JVM에서 자바의 클래스를 읽어오기 위해 자바 클래스 로더를 사용한다
    // 클래스 로더는 클래스 파일을 찾고, 메모리에 로드해 살행 가능한 상태로 만드는 역할을 한다
    // 과정
    // 1. Class Loading: 클래스 파일을 가져와 JVM메모리에 적재한다
    // 이 단계도 크게 JVM기본 클래스와 Java 코드를 로딩하는 Bootstrap Class Loading
    // 자바 핵심 라이브러리를 로딩하는 Extension Class Loading
    // 개발자가 직접 개발하여 Classpath에 있는 클래스를 로딩하는 Application Class Loading 단계로 나뉟나
    ...
    // 클래스 로더는 일반적으로 Lazy Loading방식 즉, 어플리케이션이 시작 할 때 로딩되는 것이 아니라,
    // 클래스가 필요한 시점까지 로딩을 지연하는 방식이다
    // 정리하면 클래스 로더는 클래스가 최초로 필요해진 시점에 클래스를 로딩 한다는 것이다.
    // 이게 Latency의 첫번째 이유이다
    // 배포 직후에는 대부분의 클래스들이 한번도 사용되지 않으므로 클래스 로더에 의해 메모리에 적재되지 않는 상태인데
    // 그런 상태에서 요청이 들어오게 되면 그제서야 클래스로더가 클래스를 메모리에 적재하면서 이 과정속에서 Latencyt가 발생하는 것이다
    ...
    // 이 바이트 코드는 어플리케이션에서 사용되는 다른 여러 리소스들과 함께 묶어, 실행될 수 있는 JAR 나 WAR파일로 아카이브 된다
    // 이런 빌드과정을 거쳐 JAR, WAR을 실행하면 JVM은 바이트 코드를 한줄 한줄 읽어 기계어로 번역한 뒤 실행하는데
    // 이 과정을 인터프리트라고 한다
    //
    // 그럼 왜 자바는 소스코드를 바로 기계어로 번역하지 않고 바이트 코드라는 중간 언어를 만들까
    // 자바의 핵심 철학 중 하나님 Write Once, Run Anywhere때문 자바는 플랫폼 독립전인 언어를 지향한다
    // 자바는 바이트코드를 사용함으로써 이런 문제를 해결함
    // JVM은 바이트 코드를 기계어로 번역할 때 플랫좀 종속적인 작업을 처리한다
    ...
    // JIT 컴파일러의 도입
    // 이런 인터프리팅 방식의 성능 문제 해결을 위해 도입한게 JIT컴파일
    // JIT컴파일러는 어플리케이션 실행 중 동적으로 바이트 코드를 기계어로 컴파일 한다
